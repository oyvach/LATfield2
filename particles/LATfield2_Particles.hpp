...
void Particles<part, part_info, part_dataType>::getPartNewProcess(part pcl, int *ranks)
{
    int coord[3];
    for (int i = 0; i < 3; i++)
        coord[i] = (int)(floor(pcl.pos[i] * lat_part_.size(i))) % lat_part_.size(i);

    if (pcl.pos[2] >= boxSize_[2])
        ranks[0] = parallel.grid_size()[0];
    else if (pcl.pos[2] < 0.)
    {
        if (pcl.pos[2] + boxSize_[2] >= boxSize_[2])
            ranks[0] = 0;
        else
            ranks[0] = -1;
    }
    ...
    if (pcl.pos[1] >= boxSize_[1])
        ranks[1] = parallel.grid_size()[1];
    else if (pcl.pos[1] < 0.)
    {
        if (pcl.pos[1] + boxSize_[1] >= boxSize_[1])
            ranks[1] = 0;
        else
            ranks[1] = -1;
    }
...
void Particles<part, part_info, part_dataType>::coutPart(long ID)
{
    ...
    for (x.first(); x.test(); x.next())
    {
        ...
        {
            if ((*it).ID == ID)
                cout << "Parallel ranks: (" << parallel.grid_rank()[1] << "," << parallel.grid_rank()[0] << ") ; " << part_global_info_.type_name << ": " << *it << " , lattice position:" << x << endl;
        }
        //        }
    }
}
...
void Particles<part, part_info, part_dataType>::saveHDF5(string filename_base, int fileNumber)
{
    ...
    if (fileNumber == 1)
        ...
        filename = filename_base + "_" + int2string(whichFile, 999) + ".h5";
...
void Particles<part, part_info, part_dataType>::loadHDF5(string filename_base, int fileNumber)
{
    ...
    {
        for (int i = 0; i < fileNumber; i++)
            get_fileDsc_global(filename_base + "_" + int2string(i, 999) + ".h5", fd[i]);
    ...
    for (int i = 0; i < fileNumber; i++)
    {
        ...
        if (!(localBoxOffset[1] >= fd[i].fileBoxOffset + fd[i].fileBoxSize) &&
            !(fd[i].fileBoxOffset >= localBoxOffset[1] + localBoxSize[1]))
    ...
    for (it = file_list.begin(); it != file_list.end(); it++)
    {
        ...
        if (fileNumber == 1)
            get_fileDsc_local(filename_base + ".h5", numParts_file,
                              localBoxOffset_file, localBoxSize_file, nPP);
        else
            get_fileDsc_local(filename_base + "_" + int2string((*it), 999) + ".h5",
        ...
        for (int i = 0; i < fd[(*it)].numProcPerFile; i++)
        {
                ...
                !(localBoxOffset_file[3 * i + 1] >= localBoxOffset[1] + localBoxSize[1]))
            {
                ...
                partList_offset = 0;
                for (int l = 0; l < i; l++)
                    partList_offset += numParts_file[l];
                partList = new part[partList_size];

                // cout<< "list size: "<<partList_size <<endl;

                if (fileNumber == 1)
                    get_part_sublist(filename_base + ".h5",
                                     partList_offset, partList_size, partList, part_datatype_);
                else
                    get_part_sublist(filename_base + "_" + int2string((*it), 999) + ".h5",
                                     partList_offset, partList_size, partList, part_datatype_);
