...
#ifdef FFT3D
	if(dim >= 2 &&
	   ((size[dim-1] % parallel.grid_size()[0]) != 0 ||
	    (size[dim-2] % parallel.grid_size()[1]) != 0))
	{
		if(parallel.isRoot())
		{
			cerr<<"Latfield2d::Lattice::initialize : FFT3D requires the decomposed lattice dimensions to divide the process grid"<<endl;
			cerr<<"Latfield2d::Lattice::initialize : size("<<dim-1<<")="<<size[dim-1]
			    <<" must be divisible by grid_size(0)="<<parallel.grid_size()[0]
			    <<" and size("<<dim-2<<")="<<size[dim-2]
			    <<" must be divisible by grid_size(1)="<<parallel.grid_size()[1]<<endl;
