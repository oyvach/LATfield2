...
int main(int argc, char **argv)
{
    ...
    Field<Real> density_device;
    density_device.initialize(*lat, 1);
    density_device.alloc(-1, Field<Real>::device);

    size_t density_bytes = (size_t)lat->sitesLocalGross() * sizeof(Real);
    auto halo_copy_status = cudaMemcpy(density_device.data(), density_new.data(), density_bytes, cudaMemcpyDefault);
    if (halo_copy_status != cudaSuccess)
    {
        cout << "Error: Cannot initialize device halo test field: " << cudaGetErrorString(halo_copy_status) << endl;
        return 1;
    }

    density_device.updateHalo();

    std::vector<Real> density_device_host(lat->sitesLocalGross());
    halo_copy_status = cudaMemcpy(density_device_host.data(), density_device.data(), density_bytes, cudaMemcpyDefault);
    if (halo_copy_status != cudaSuccess)
    {
        cout << "Error: Cannot copy device halo test field to host: " << cudaGetErrorString(halo_copy_status) << endl;
        return 1;
    }
    ...
    for (x.haloFirst(); x.haloTest(); x.haloNext())
    {
        ...
        if (fabs((density_old(x) + tolerance) / (density_device_host[x.index()] + tolerance) - 1.0) > tolerance)
        {
            cout << "Error: Device halo update differs at site " << x.coord(0) << " " << x.coord(1) << " " << x.coord(2) << " --- Old: " << density_old(x) << " Device: " << density_device_host[x.index()] << endl;
            return 1;
        }
    }
