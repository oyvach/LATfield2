...
__device__ inline void call_lattice_functor(ForEachFunct &funct, Field<Real> **fields, Site *sites, int nfields,
                                            double *params, double *output_site, void **vparams, int step)
{
    if constexpr (std::is_invocable_v<ForEachFunct &, Field<Real> **, Site *, int, double *, double *, void **, int>)
    {
        funct(fields, sites, nfields, params, output_site, vparams, step);
    }
    else if constexpr (std::is_invocable_v<ForEachFunct &, Field<Real> **, Site *, int, double *, double *, void **>)
    {
        funct(fields, sites, nfields, params, output_site, vparams);
    }
    else
    {
        funct(fields, sites, nfields, params, output_site);
    }
}
...
__global__ void lattice_for_each(ForEachFunct funct, int numpts, Field<Real> ** fields, int nfields, double * params,
                                double * output, int * reduce_type, void ** vparams = nullptr, const int intoHalo = 0,
                                const int step = -1)
{
