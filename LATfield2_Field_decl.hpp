...
void Field<FieldType>::updateHaloComms()
{
	...
	if(parallel.grid_rank()[1]%2==0)
	{
		...
		else
		{
			...
			copy_halo_values<<<lattice_->sizeLocal(lattice_->dim()-1), 128>>>(pointer_send_up, buffer_send, lattice_->jump(lattice_->dim()-1)*components_, buffer_size1/lattice_->sizeLocal(lattice_->dim()-1), buffer_size1/lattice_->sizeLocal(lattice_->dim()-1));
		...
		else
		{
			...
			copy_halo_values<<<lattice_->sizeLocal(lattice_->dim()-1), 128>>>(buffer_rec, pointer_rec_up, buffer_size1/lattice_->sizeLocal(lattice_->dim()-1), lattice_->jump(lattice_->dim()-1)*components_, buffer_size1/lattice_->sizeLocal(lattice_->dim()-1));
			nvtxRangePop();
		}
		...
		{
			nvtxRangePushA("copy halo values (device)");
			copy_halo_values<<<lattice_->sizeLocal(lattice_->dim()-1), 128>>>(buffer_rec, pointer_rec_down, buffer_size1/lattice_->sizeLocal(lattice_->dim()-1), lattice_->jump(lattice_->dim()-1)*components_, buffer_size1/lattice_->sizeLocal(lattice_->dim()-1));
	...
	if(parallel.grid_rank()[0]%2==0)
	{
		if(parallel.grid_rank()[0]!=parallel.grid_size()[0]-1)
		{
			...
			if (use_device_copy) // can't just send/rec into the data array, use send/rec buffers
			{
				...
				copy_halo_values<<<buffer_size0/lattice_->jump(lattice_->dim()-2), 128>>>(buffer_rec, pointer_rec_up, lattice_->jump(lattice_->dim()-2), lattice_->jump(lattice_->dim()-2), lattice_->jump(lattice_->dim()-2));
		...
		if (use_device_copy && (parallel.grid_rank()[0] != 0 || parallel.grid_size()[0]%2==0))
		{
			...
			copy_halo_values<<<buffer_size0/lattice_->jump(lattice_->dim()-2), 128>>>(buffer_rec, pointer_rec_down, lattice_->jump(lattice_->dim()-2), lattice_->jump(lattice_->dim()-2), lattice_->jump(lattice_->dim()-2));
