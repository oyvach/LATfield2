...
// Helper function to infer the alignment of a pointer in memory

size_t get_pointer_alignment(void* ptr)
{
...
extern constexpr int symmetric = 1;
extern constexpr int unsymmetric = 0;
...
class Field
	...
	public:
		...
		void alloc(long size, int alloc_type);
        /*!
    ...
    bool IsInitialized() { return (status_ & initialized) > 0; }
         ...
         */
		__host__ __device__ FieldType& operator()(long index);
         ...
         */
		__host__ __device__ FieldType& operator()(long index, int component);
         ...
         */
		__host__ __device__ FieldType& operator()(long index, int i, int j);
         ...
         */
		__host__ __device__ FieldType& operator()(const Site& site);
        ...
        /*!
         ...
         \sa To have more description see the Site class documentation.
         */
		__host__ __device__ FieldType& operator()(const Site& site, int component);
        ...
        /*!
         ...
         \sa To have more description see the Site class documentation.
         */
		__host__ __device__ FieldType& operator()(const Site& site, int i, int j);
		...
		size_t temporaryHaloWorkspaceBytes() const;
	    ...
	    */
		__host__ __device__ Lattice& lattice();
        /*!
         Returns the number of components of the field at each sites.
         */
		__host__ __device__ int components();
        /*!
    ...
    public:
        FieldType* data_;
		static constexpr int host = 0;    // Host (CPU) memory allocation
		static constexpr int device = 1;  // Device (GPU) memory allocation
		static constexpr int managed = 2; // Managed (unified) memory allocation
	protected:
		//MEMBER DATA
		Lattice*   lattice_;
    ...
    int        matrixSize_;
		...
		int        status_;
		int        alloc_type_;
		static constexpr int initialized = 1;
...
void Field<FieldType>::alloc()
{
    ...
    size_t alignment = get_pointer_alignment(data_);
    std::cerr << "LATField2d::Field::alloc()  :proc#" << parallel.rank() << " allocated field data array with alignment: " << alignment << std::endl;
#endif
...
#ifdef DEBUG_ALIGNMENT
    size_t alignment = get_pointer_alignment(data_);
    std::cerr << "LATField2d::Field::alloc(long size)  :proc#" << parallel.rank() << " allocated field data array with alignment: " << alignment << std::endl;
#endif
    }
...
template <class FieldType>
void Field<FieldType>::alloc(long size, int alloc_type)
{
	if (alloc_type == device || alloc_type == managed)
	{
		...
		{
			cout << "LATField2d::Field::alloc(long size, int alloc_type)  :process " << parallel.rank() << " cannot allocate the field data array." << endl;
			data_memSize_ = 0;
			status_ = status_ ^ allocated;
...
void Field<FieldType>::dealloc()
{
	if((status_ & allocated) > 0)
    {
		if (alloc_type_ == device || alloc_type_ == managed)
		{
			cudaFree(data_);
		}
		else
...
template <class FieldType>
__host__ __device__ inline FieldType& Field<FieldType>::operator()(long index)
{
	return data_[index];
}

template <class FieldType>
__host__ __device__ inline FieldType& Field<FieldType>::operator()(long index, int component)
{
	return data_[index*components_ + component];
}

template <class FieldType>
__host__ __device__ inline FieldType& Field<FieldType>::operator()(long index, int i, int j)
{
	int component;
...
template <class FieldType>
__host__ __device__ inline FieldType& Field<FieldType>::operator()(const Site& site)
{
	return this->operator()(site.index());
}

template <class FieldType>
__host__ __device__ inline FieldType& Field<FieldType>::operator()(const Site& site, int component)
{
	return this->operator()(site.index(),component);
}

template <class FieldType>
__host__ __device__ inline FieldType& Field<FieldType>::operator()(const Site& site, int i, int j)
{
...
template <class FieldType>
__global__ void copy_halo_values (FieldType * data, int size_1, int jump_1, int jump_2, int halo, int components)
{
	int start = (blockIdx.x + halo) * jump_2 + halo * jump_1;

	for (int i = threadIdx.x/2; i < size_1; i += 64)
	{
		if (threadIdx.x % 2 == 0)
		{
			for (int j = 0; j < components * halo; j++)
			{
				data[start + i * jump_1 + j] = data[start  + i * jump_1 + j + (jump_1 - 2 * halo * components)];
			}
		}
		else
		{
			for (int j = 0; j < components * halo; j++)
			{
				data[start + i * jump_1 + j + (jump_1 - halo * components)] = data[start + i * jump_1 + j + halo * components];
			}
		}
	}
}
...
template <class FieldType>
size_t Field<FieldType>::temporaryHaloWorkspaceBytes() const
{
	if ((status_ & initialized) == 0 || lattice_ == nullptr) return 0;

	size_t buffer_size1 = (size_t)lattice_->halo()
	                    * (size_t)components_
	                    * (size_t)lattice_->jump(lattice_->dim()-2)
	                    * (size_t)lattice_->sizeLocal(lattice_->dim()-1);
	return 2 * buffer_size1 * sizeof(FieldType);
}

template <class FieldType>
__global__ void copy_halo_values (FieldType * src, FieldType * dest, int stride_src, int stride_dest, int i_max)
{
	for (int i = threadIdx.x; i < i_max; i += 128)
	{
...
void Field<FieldType>::updateHaloComms()
{
	...
	const bool non_host_data = attributes.type != cudaMemoryTypeHost;
	...
	const bool cuda_aware_mpi_active =
		...
		cuda_aware_mpi_hint;
	const bool stage_dim0_device_buffers = non_host_data && !cuda_aware_mpi_active;

	auto push_halo_mpi_range = [&](const char* host_staged_label)
	{
		nvtxRangePushA(cuda_aware_mpi_active ? "Field::updateHaloComms : CUDA-aware MPI halo exchange" : host_staged_label);
	};
	auto push_halo_device_copy_range = [&]()
	{
		nvtxRangePushA(cuda_aware_mpi_active ? "Field::updateHaloComms : CUDA-aware halo device pack/unpack" : "copy halo values (device)");
	};
	...
	bool private_device_buffers = false;
	void* device_buffer_block = nullptr;
	...
	else if (cuda_aware_mpi_active)
	{
		size_t buffer_bytes = (size_t)buffer_size1 * sizeof(FieldType);
		size_t workspace_bytes = 2 * buffer_bytes;

		nvtxRangePushA("Field::updateHaloComms : CUDA-aware halo device workspace");
#ifdef FFT3D
		tempMemory.reserveDeviceWorkspaceBytes(workspace_bytes, "Field::updateHaloComms");
		device_buffer_block = tempMemory.deviceWorkspace();
#else
		if (workspace_bytes > 0)
		{
			success = cudaMalloc(&device_buffer_block, workspace_bytes);
			if (success != cudaSuccess)
			{
				nvtxRangePop();
				cout << "LATfield2::Field::updateHaloComms  :process " << parallel.rank() << " cannot allocate CUDA-aware halo buffers." << endl;
				throw std::runtime_error("CUDA halo buffer allocation failed");
			}
			private_device_buffers = true;
		}
#endif
		nvtxRangePop();

		buffer_send_dev = reinterpret_cast<FieldType*>(device_buffer_block);
		buffer_rec_dev = (buffer_send_dev == nullptr) ? nullptr : buffer_send_dev + buffer_size1;
		buffer_send = buffer_send_dev;
		buffer_rec = buffer_rec_dev;
	}
	else
	{
	...
	if(parallel.grid_rank()[1]%2==0)
	{
		...
		push_halo_mpi_range("send halo values");
		...
		push_halo_mpi_range("receive halo values");
		if(parallel.grid_rank()[1]!=parallel.grid_size()[1]-1)
		{
		...
		{
			push_halo_device_copy_range();
			success = cudaDeviceSynchronize(); // finalize next send buffer copy
	...
	if(parallel.grid_rank()[0]%2==0)
	{
		...
		{
			if (stage_dim0_device_buffers) // host-staged device buffers keep MPI off the field data
			{
				push_halo_device_copy_range();
			...
			push_halo_mpi_range("send/rec halo values");
		...
		if (stage_dim0_device_buffers && (parallel.grid_rank()[0] != 0 || parallel.grid_size()[0]%2==0))
		{
			push_halo_device_copy_range();
	...
	else
	{
		...
		push_halo_mpi_range("rec/send halo values");
		if(parallel.grid_rank()[0]!=parallel.grid_size()[0]-1)
		{
			parallel.receive_dim0( pointer_rec_up, buffer_size0, parallel.grid_rank()[0]+1);
			parallel.send_dim0( pointer_send_up, buffer_size0, parallel.grid_rank()[0]+1);
	...
	{
		if (private_device_buffers) cudaFree(device_buffer_block);
	}
	else if (buffers_mapped)
	{
		cudaFreeHost(buffer_send);
		cudaFreeHost(buffer_rec);
	}
...
void Field<FieldType>::saveSliceHDF5_coarseGrain2D(string filename, string dataset_name, int xcoord, int thickness, int ratio)
{
	...
	sfield.saveSliceHDF5(filename, dataset_name, xcoord, thickness);
	sfield.dealloc();
#else
	COUT << "LATfield2d must be compiled with HDF5 (flag HDF5 turn on!!)" << endl;
	COUT << "to be able to use hdf5 data format!!!)" << endl;
...
template <class FieldType>
__host__ __device__ Lattice& Field<FieldType>::lattice() { return *lattice_; }

template <class FieldType>
__host__ __device__ int Field<FieldType>::components() { return components_; }
