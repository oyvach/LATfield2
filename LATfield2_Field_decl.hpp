...
inline int getCompSym(int i, int j)
{
	int component;
	if (i > j)
		component = i + j * 3 - (j * (1 + j)) / 2;
	else
		component = j + i * 3 - (i * (1 + i)) / 2;
	return component;
}
...
public:
	...
	/*!
	  Modified function to does both slice and coarse grain
	 */
	void saveSliceHDF5_coarseGrain2D(string filename, string dataset_name, int xcoord, int thickness = 1, int ratio = 1);
	void saveSliceHDF5_coarseGrain2D(string filename, int xcoord, int thickness = 1, int ratio = 1)  { this->saveSliceHDF5(filename, "data", xcoord, thickness); }
...
void Field<FieldType>::alloc(long size)
{
	...
	if (size < lattice_->sitesLocalGross())
		alloc_number = lattice_->sitesLocalGross();
	else
		alloc_number = size;
...
inline FieldType &Field<FieldType>::operator()(long index, int i, int j)
{
	...
	{
		if (i > j)
			component = i + j * rows_ - (j * (1 + j)) / 2;
		else
			component = j + i * rows_ - (i * (1 + i)) / 2;
	}
	else
	{
		component = j * rows_ + i;
...
void Field<FieldType>::write(const string filename)
{
	...
	int start, stop;
	int haloreverse = 0;
	for (i = 0; i < lattice_->dim() - 2; i++)
		haloreverse += lattice_->jump(i);
...
void Field<FieldType>::read(const string filename)
{
	...
	for (p = 0; p < parallel.size(); p++)
	{
		if (parallel.rank() == p)
		{
			...
			stop = start + (lattice_->jump(lattice_->dim() - 2)) * lattice_->sizeLocal(lattice_->dim() - 2) - 2 * haloreverse * lattice_->halo();
			skip = 1;
			for (i = 0; i < lattice_->dim() - 1; i++)
				skip *= lattice_->size()[i];
...
void Field<FieldType>::saveSliceHDF5(string filename, string dataset_name, int xcoord, int thickness)
{
	...
	if (thickness > 1)
	{
		...
		for (sX.first(); sX.test(); sX.next())
		{
			...
			for (int i = 0; i < components_; i++)
				sfield(sX, i) = this->operator()(X, i);
		}
	}
	else
	{
		for (int i = 0; i < dim - 1; i++)
			sSize_thin[i] = this->lattice_->size(i + 1);
...
void Field<FieldType>::saveHDF5_server_open(string filename_base, int offset, int thickness)
{
	...
	for (int i = 0; i < lattice_->dim(); i++)
		size[i] = lattice_->size(i);
	iof_offset_ = offset;
	if (iof_offset_ != -1 && thickness == -1)
		iof_thickness_ = 1;
	else
		iof_thickness_ = thickness;
	if (lattice_->dim() == 2)
		iof_thickness_ = -1;
	if (iof_thickness_ != -1)
...
void Field<FieldType>::saveHDF5_server_write(int number_of_message, string filename_base, int offset, int thickness)
{
	...
	if (!(io_file_.is_open))
		this->saveHDF5_server_open(filename_base, offset, thickness);

	if (!(io_file_.is_open))
	{
		COUT << "Field<FieldType>::saveHDF5_server_write cannot open file: " << filename_base << endl;
		return;
	}
	...
	else
	{
		...
		if (abs(number_of_message - nom_max) <= abs(number_of_message - nom_min))
			nom = nom_max;
		else
			nom = nom_min;
	...
	{
		if (!x.setCoord(coord))
			cout << "internal error: ioserver send field out of local space..." << endl;
			...
			if (iof_thickness_ == -1)
			{
				for (int i = 0; i < bufSize; x.next(), i++)
				{
					for (int c = 0; c < components_; c++)
						buffer[i * components_ + c] = data_[x.index() * components_ + c];
				}
			}
			else
			{
